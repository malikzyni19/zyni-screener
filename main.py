...
def detect_obs_all(o, h, l, c, v, i_len, s_len, max_ob=20):
    """
    Like detect_obs but returns ALL OBs including mitigated ones.
    Used by detect_breakers to find OBs that have been broken through.
    """
    n = len(c)
    if n < max(i_len, s_len) + 5:
        return []

    # Use same internal logic as detect_obs but skip mitigation filter
    # Re-use detect_obs internals by calling it with a modified candle set
    # Strategy: call detect_obs on the first portion of data where OB was active
    # Simpler approach: detect all OBs, check mitigation separately in detect_breakers

    obs = []
    # Find swing highs and lows using same logic as detect_obs
    for i in range(s_len, n - 1):
        # Bearish OB: last red candle before a bullish impulse (swing high)
        is_swing_high = all(h[i] >= h[i - k] for k in range(1, min(s_len, i) + 1)
                           if i - k >= 0)
        if is_swing_high:
            # Find last bearish candle before this swing high
            for j in range(i - 1, max(0, i - i_len - 1), -1):
                if c[j] < o[j]:  # bearish candle
                    vol = sum(v[j:i+1]) / max(1, i - j + 1)
                    buy_vol  = vol * 0.4
                    sell_vol = vol * 0.6
                    obs.append({
                        "type": "bearish", "bar": j,
                        "top": h[j], "bottom": min(o[j], c[j]),
                        "avg": (h[j] + min(o[j], c[j])) / 2,
                        "volume": vol * (i - j + 1),
        ...
        # Bullish OB: last green candle before a bearish impulse (swing low)
        is_swing_low = all(l[i] <= l[i - k] for k in range(1, min(s_len, i) + 1)
                          if i - k >= 0)
        if is_swing_low:
            for j in range(i - 1, max(0, i - i_len - 1), -1):
                if c[j] > o[j]:  # bullish candle
                    ...
                    obs.append({
                        "type": "bullish", "bar": j,
                        "top": max(o[j], c[j]), "bottom": l[j],
                        "avg": (max(o[j], c[j]) + l[j]) / 2,
                        "volume": vol * (i - j + 1),
                        "buyVolume": buy_vol, "sellVolume": sell_vol,
                        "strengthPct": 50.0,
                    })
    ...
    unique_obs = []
    for ob in obs:
        if ob["bar"] not in seen_bars:
            seen_bars.add(ob["bar"])
            unique_obs.append(ob)

    # Sort by bar descending (most recent first) and limit
    unique_obs.sort(key=lambda x: x["bar"], reverse=True)
    return unique_obs[:max_ob]
...
def detect_breakers(o: List[float], h: List[float], l: List[float], c: List[float],
                    v: List[float], price: float, tf: str,
                    approach_pct: float = 2.0, max_age: int = 50,
                    i_len: int = 5, s_len: int = 30,
                    fvgs: Optional[List[Dict]] = None) -> List[Dict[str, Any]]:
    """
    ...
    for ob in raw_obs:
        ...
        if ob_bar < 0 or ob_bar >= n:
            continue

        # Scan forward from OB formation to find mitigation
        mitigated      = False
        mitigated_bar  = -1
        consumed       = False

        for i in range(ob_bar + 1, n):
            if not mitigated:
                # Check if OB is mitigated (flips to breaker)
                if is_bull and c[i] < ob_bottom:
                    mitigated     = True
                    mitigated_bar = i
                elif not is_bull and c[i] > ob_top:
                    mitigated     = True
                    mitigated_bar = i
            else:
                # Already a breaker — check if consumed
                # Bullish breaker: price was bearish OB, flipped bullish
                # Consumed when price drops below bottom again
                if is_bull:
                    # Was bullish OB → now bearish breaker
                    # Consumed when close > ob_top (price goes back above)
                    if c[i] > ob_top:
                        consumed = True
                        break
                else:
                    # Was bearish OB → now bullish breaker
                    # Consumed when close < ob_bottom
                    if c[i] < ob_bottom:
                        consumed = True
                        break

        if not mitigated or consumed:
            continue

        # Check age — how many bars since mitigation
        age = (n - 1) - mitigated_bar
        if age > max_age:
            continue

        # Determine breaker direction (flipped from original OB)
        breaker_dir = "bearish" if is_bull else "bullish"
        ...
        if breaker_dir == "bearish":
            ...
            if price <= ob_top:
                dist_pct = (ob_bottom - price) / max(price, 1e-10) * 100
                dist_pct = max(dist_pct, 0)
            else:
                dist_pct = (price - ob_top) / max(price, 1e-10) * 100
        else:
        ...
        # State
        price_in_zone = ob_bottom <= price <= ob_top
        if price_in_zone:
            state = "inside"
        ...
        # Strength inherited from original OB
        ...
        if fvgs:
            for fvg in fvgs:
                ...
                fb = fvg.get("bottom", 0)
                if ft > 0 and fb > 0:
                    # Check if FVG overlaps breaker zone
                    if ft >= ob_bottom and fb <= ob_top:
                        fvg_overlap = True
                        break
        ...
        breakers.append({
            ...
            "zone_str":     f"{fmt_price(ob_bottom)} – {fmt_price(ob_top)}",
            "orig_type":    ob["type"],
            "bar":          ob_bar,
            "mitigation_bar": mitigated_bar,
        })
...
def score_ob_quality(ob: Dict[str, Any], o: List[float], h: List[float],
...
def analyze_pair(symbol: str, candles: List[Dict[str, float]], tf: str, settings: Dict[str, Any], btc_closes: Optional[List[float]] = None, fib_candles: Optional[List[Dict[str, float]]] = None) -> Optional[Dict[str, Any]]:
    ...
    if settings.get("useBreakerModule"):
        ...
        brk_max_age  = int(settings.get("breakerMaxAge", 50))
        brk_req_fvg  = bool(settings.get("breakerRequireFvg", False))
        # Always detect FVGs fresh — fvg_list not available in this scope
        ...
        for brk in breakers:
            ...
            if brk["state"] not in ("approaching", "inside"):
                continue
            setup_name = "BREAKER_APPROACH" if brk["state"] == "approaching" else "BREAKER_INSIDE"
            ...
            alerts.append({
                ...
                "detail": (
                    ...
                    f'{" | FVG ✅" if high_prob else ""}'
                ),
                "strength": 5 if high_prob else 4,
                "meta": {
                    "breakerTop":    brk["top"],
...
def parse_settings(payload: Dict[str, Any]) -> Dict[str, Any]:
    return {
        ...
        "breakerMaxAge":     int(payload.get("breakerMaxAge", 50)),
...
def _scan_pair_multitf(symbol: str, market: str = "perpetual", wl_config: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    breaker_approach_pct = cfg.get("breaker_approach_pct", 2.0)
    breaker_max_age      = cfg.get("breaker_max_age", 50)
    breaker_require_fvg  = cfg.get("breaker_require_fvg", False)
    ...
    for tf in tfs:
        try:
            ...
            # ── Breaker Block detection ──
            if scan_breaker:
                fvgs_for_brk = detect_fvgs(o, h, l, c, v, tf)
                raw_breakers = detect_breakers(
                    o, h, l, c, v, price, tf,
                    approach_pct=breaker_approach_pct,
                    max_age=breaker_max_age,
                    i_len=settings["iLen"],
                    s_len=settings["sLen"],
                    fvgs=fvgs_for_brk,
                )
                for brk in raw_breakers:
                    if breaker_require_fvg and not brk["fvg_overlap"]:
                        continue
                    tf_breakers.append({
                        "tf":        tf,
                        "direction": brk["type"],
                        ...
                        "fvg_overlap": brk["fvg_overlap"],
                        "zone_str":  brk["zone_str"],
...
def api_bias_scan():
    ...
    pairs_per_cycle = int(payload.get("pairsPerCycle", 20))
    use_rr          = payload.get("roundRobin", False)
    ...
    if scan_mode == "market":
        ...
        # No pairs selected — return empty
        return jsonify([])
