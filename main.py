...
def detect_obs_all(o, h, l, c, v, i_len, s_len, max_ob=20):
    ...
    Used exclusively by detect_breakers.
...
def detect_breakers(o: List[float], h: List[float], l: List[float], c: List[float],
                    ...
                    approach_pct: float = 2.0, max_age: int = 200,
...
def analyze_pair(symbol: str, candles: List[Dict[str, float]], tf: str, settings: Dict[str, Any], btc_closes: Optional[List[float]] = None, fib_candles: Optional[List[Dict[str, float]]] = None) -> Optional[Dict[str, Any]]:
    ...
    obs = detect_obs(o, h, l, c, v, settings["iLen"], settings["sLen"], max_ob=15)
    ...
    ob_approach_pct_base = settings.get("obDistancePct", settings.get("approachPct", 5.0))
    ob_consol_tol_pct    = min(ob_approach_pct_base, 0.50)
    needed_consol        = settings.get("consolCandles", 3)
    ...
    for direction, ob_list in [("bullish", bullish_obs_filt), ("bearish", bearish_obs_filt)]:
        ...
        for ob in ob_list:  # ← check ALL OBs, not just nearest
            ...
            if dist_pct <= ob_approach_pct and correct_side:
                alerts.append({
                    "setup": "OB_APPROACH",
                    "direction": direction,
                    "timeframe": tf,
...
_WL_SCAN_SETTINGS: Dict[str, Any] = {
    "iLen": 5, "sLen": 30,
    "approachPct": 5.0, "obDistancePct": 5.0,
...
def _scan_pair_multitf(symbol: str, market: str = "perpetual", wl_config: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    breaker_max_age      = cfg.get("breaker_max_age", 200)
    breaker_require_fvg  = cfg.get("breaker_require_fvg", False)
    tfs        = cfg.get("timeframes", ["15m", "1h", "4h"])
    ob_appr    = cfg.get("ob_approach", {"15m": 2.0, "1h": 3.0, "4h": 5.0, "30m": 2.5})
    ...
    for tf in tfs:
        try:
            ...
            if scan_ob:
                iLen = settings["iLen"]
                sLen = settings["sLen"]
                raw_obs = detect_obs(o, h, l, c, v, iLen, sLen, max_ob=15)
