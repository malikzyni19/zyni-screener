...
def get_ob_zone_levels(symbol: str, zone_top: float, zone_bottom: float,
                        ob_type: str) -> Dict[str, Any]:
    """
    Read levels from in-memory full order book for a specific zone.
    Returns all orders at ANY price level — no range limitation.
    """
    with _ob_book_lock:
        book = _ob_books.get(symbol)

    if not book or not book.get("ready"):
        return {"ready": False, "levels": [], "error": "book not ready"}

    side = "bids" if ob_type == "bullish" else "asks"
    all_levels = book[side]

    # All order book levels — no range limitation
    all_prices = sorted(all_levels.keys(), reverse=(ob_type == "bearish"))
    all_qtys   = list(all_levels.values())
    avg_qty    = sum(all_qtys) / max(len(all_qtys), 1) if all_qtys else 1.0

    # Filter zone levels — exact zone match
    zone_buffer  = abs(zone_top - zone_bottom) * 0.5
    zone_levels  = [(p, all_levels[p]) for p in all_prices
                    if (zone_bottom - zone_buffer) <= p <= (zone_top + zone_buffer)]

    # Note whether we found in-zone or fell back
    zone_note = "in_zone" if any(zone_bottom <= p <= zone_top for p, _ in zone_levels) else "near_zone"

    # If nothing near zone — show nearest 15 levels to zone
    if not zone_levels:
        zone_mid = (zone_top + zone_bottom) / 2
        zone_levels = sorted(all_levels.items(), key=lambda x: abs(x[0] - zone_mid))[:15]
        zone_note   = "nearest_to_zone"

    def _classify(qty):
        r = qty / max(avg_qty, 1e-10)
    ...
    for p, q in sorted(zone_levels, key=lambda x: x[0], reverse=(ob_type == "bearish")):
        cls, icon = _classify(q)
        usdt_val  = q * p
        levels_out.append({
            "price":   round(p, 8),
            "qty":     round(q, 4),
    ...
    zone_label = {"in_zone": "in zone", "near_zone": "near zone", "nearest_to_zone": "nearest to zone"}[zone_note]
...
def _scan_pair_multitf(symbol: str, market: str = "perpetual", wl_config: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    for tf in tfs:
        try:
            candles = get_klines(symbol, tf, 200, market)
            ...
            for alert in tf_result.get("alerts", []):
                ...
                if scan_ob and setup.startswith("OB_"):
                    tf_obs.append({
                        "tf":           tf,
                        "direction":    direction,
                        "setup":        setup,
                        "top":          meta.get("obTop", 0),
                        "bottom":       meta.get("obBottom", 0),
                        "strength":     meta.get("obStrengthPct", 0),
                        "quality":      meta.get("obQuality", 0),
                        "qualityLabel": meta.get("obQualityLabel", ""),
                        "dist":         meta.get("obDistPct", 0),
                        "state":        meta.get("obState", ""),
                        "absorption":   meta.get("absorption", "NONE"),
                        "absorptionStr": meta.get("absorptionStr", ""),
                        "checklist":    {
                            "sweep": meta.get("sweepPass", False),
                            "disp":  meta.get("dispPass", False),
                            "fvg":   meta.get("fvgPass", False),
                            "pd":    meta.get("pdPass", False),
                            "htf":   meta.get("htfPass", False),
                            "safe":  meta.get("safePass", False),
                            "abs":   meta.get("absorptionPass", False),
                        },
                        "zone_str": _extract_zone_from_detail(alert.get("detail", "")),
                        "detail":   alert.get("detail", ""),
                    })

                elif scan_fvg and setup == "FVG":
            ...
            result["tfs"][tf] = {
                "score":      tf_result.get("score", 0),
                "confidence": tf_result.get("confidence", ""),
                "trend":      tf_result.get("trend", 0),
                "itrend":     tf_result.get("itrend", 0),
                "rsi":        tf_result.get("rsi", 0),
                "atr":        tf_result.get("atr", 0),
                "obs":        tf_obs,
                "fvgs":       tf_fvgs,
                "fibs":       tf_fibs,
            }
            ...
            result["rsi"][tf] = tf_result.get("rsi", 0)
            result["atr"][tf] = tf_result.get("atr", 0)
    ...
    if book_ready:
        ...
        if ob_result.get("ready"):
            return jsonify({
                ...
                "zone_bottom":   round(zone_bottom, 8),
                "side":          "bids" if ob_type == "bullish" else "asks",
                "levels":        ob_result["levels"],
                "total_usdt":    ob_result["total_usdt"],
                "total_fmt":     ob_result["total_fmt"],
                "extreme_count": ob_result["extreme_count"],
                "heavy_count":   ob_result["heavy_count"],
                "moderate_count": ob_result["moderate_count"],
                "avg_book_qty":  0,
                "verdict":       ob_result["verdict"],
                "verdict_desc":  ob_result["verdict_desc"],
                "verdict_color": ob_result["verdict"].lower(),
                "zone_note":     ob_result["zone_note"],
                "source":        f"live_ws · {ob_result['total_bids']} bids / {ob_result['total_asks']} asks · {ob_result['book_age_sec']}s old",
            })
...
def api_fvg_imbalance():
    ...
    sell = total - taker_buy

    We use the 3 candles that FORMED the FVG:
    candle[i-2], candle[i-1] (middle/imbalance), candle[i]
    """
    data    = request.get_json(force=True) or {}
    symbol  = data.get("symbol", "").strip().upper()
    fvgs_in = data.get("fvgs", [])
    tf      = data.get("tf", "15m")  # timeframe hint
    ...
    try:
        ...
        def _analyze_fvg(fvg: Dict) -> Dict:
            ...
            # ── Method 1: Find the 3 formation candles by price overlap ──
            # The FVG is formed by 3 candles where candle[i-2] and candle[i]
            # don't overlap. We look for candles whose price range touches the zone.
            # The middle candle (i-1) is the impulse candle.

            # Try to find formation candles based on age
            # age=0 means just formed, age=3 means 3 candles ago
            formation_idx = max(0, len(candles_data) - 1 - age)

            # Look for the 3 candles around formation_idx
            bv = sv = 0.0
            has_data = False

            # Primary: use formation candles (middle candle = impulse)
            for offset in range(-1, 4):  # check a few candles around formation
                idx = formation_idx + offset
                if idx < 0 or idx >= len(candles_data):
                    continue
                c = candles_data[idx]
                # Check if this candle's range overlaps with FVG zone
                if c["high"] >= bottom and c["low"] <= top:
                    bv += c["buy"]
                    sv += c["sell"]
                    has_data = True

            # Fallback: if no candles found by overlap, use formation candles by index
            ...
            total    = bv + sv
            buy_pct  = round(bv / total * 100, 1) if total > 0 else 0.0
            sell_pct = round(sv / total * 100, 1) if total > 0 else 0.0

            # Which side dominated
            dom_pct = buy_pct if direction == "bullish" else sell_pct

            # Strength classification
            if dom_pct >= 85:   strength = "EXTREME"
            elif dom_pct >= 70: strength = "STRONG"
            elif dom_pct >= 60: strength = "MODERATE"
            else:               strength = "WEAK"
